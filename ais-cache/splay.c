...
extern struct spblk * _sp_enq(struct spblk *, struct sptree *); /* insert item into the tree */
extern struct spblk * _sp_deq(struct spblk **); /* return and remove lowest item in subtree */
extern void _splay(struct spblk *, struct sptree *); /* reorganize tree */
...
struct spblk *sp_alloc_blk(void)
{
	...
	if (!sp_free_list) {
		...
		for (i = 0; i < SP_ALLOC_N; i++) {
			p = (struct spblk *) malloc(sizeof(*p));
...
void sp_scan(int (*f)(struct spblk *), struct spblk *n, struct sptree *q)
{
...
struct sptree *sp_init()
{
	...
	q = (struct sptree *) malloc(sizeof(*q));
